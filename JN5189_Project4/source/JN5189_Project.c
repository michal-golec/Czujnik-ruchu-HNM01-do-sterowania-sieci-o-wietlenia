// WERSJA Michała Zbiecia [ORYGINAL] samo max, min
#include <stdio.h>
#include "board.h"
...
#define THRESHOLD 200	//próg przełączania
    ...
    while(1) {
    	...
    	if (processDataFlag) {
			...
			if (printDelayCounter >= 0) { // print co 10 000 probek
				PRINTF("Max: %u | Min: %u | Roznica: %u | Prog: %u | Stan: %u\r\n", localMax, localMin, finalVal, THRESHOLD, state);
				printDelayCounter = 0;
			}
		}
		__WFI();

...
void ADC0_SEQA_IRQHandler(void) {
	...
	if (ADC_GetChannelConversionResult(ADC_BASE, ADC_CHANNEL, &adcResultInfoStruct)) {
		...
		if (isFirstSample) {
            ...
            filteredAdcValue = ((filteredAdcValue * 127) + adcResultValue) / 128;
//			filteredAdcValue = (adcResultValue >= filteredAdcValue) ? (filteredAdcValue = adcResultValue) : (filteredAdcValue = filteredAdcValue - 5);
		}
